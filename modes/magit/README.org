...
Everything is contained in evil-collection-magit.el, so you may download and load that file
directly. The recommended method is to use MELPA via package.el (=M-x
package-install RET evil-collection-magit RET=).
...
'evil-collection-magit)= in your configuration.
