...
;; ;; (setq evil-collection-magit-state 'motion)
;; (require 'evil-collection-magit)

;; Use `evil-collection-magit-revert` to revert changes made by
;; evil-collection-magit to the default evil+magit behavior.
...
(defun evil-collection-magit-define-key (state map-sym key def)
...
should be a string suitable for `kbd'."
  (if (and (boundp map-sym)
      ...
      (define-key
        ...
        (kbd key) def)
    (message "evil-collection-magit: Error the keymap %s is not bound. Note evil-collection-magit assumes\
...
(dolist (binding evil-collection-magit-mode-map-bindings)
  (when binding
    (dolist (state (nth 0 binding))
      (evil-collection-magit-define-key
...
(eval-after-load 'git-rebase
  `(progn
     ...
     (dolist (cmd evil-collection-magit-rebase-commands-w-descriptions)
       (when (car cmd)
         (evil-collection-magit-define-key evil-collection-magit-state 'git-rebase-mode-map
                                           (car cmd) (nth 1 cmd))))
